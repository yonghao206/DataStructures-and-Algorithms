    ...
    def __str__(self):
        ...
        for i in range(len(self._tree)):
            ...
            if i != len(self._tree) - 1:
                res.append(', ')
        res.append(']')
        return '.segment_tree.SegmentTree: ' + ''.join(res)
...
if __name__ == '__main__':
    nums = [-2, 0, 3, -5, 2, -1,10, 124, 12,31,65,82,334]
    sum_merger = lambda a, b: a + b #这里传入了一个add的merge方法，
    seg_tree = SegmentTree(arr=nums, merger=sum_merger)
    print(seg_tree)
    print(seg_tree.query(0, 3))
    print(seg_tree.query(3, 3))
    print(seg_tree.query(4,4))