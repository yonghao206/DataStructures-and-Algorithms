...
class ArrayQueue(QueueBase):
    ...
    def dequeue(self): #O(n) need to improve 
    ...
    def __str__(self):
        return str('queue.ArrayQueue : {}'.format(self._array))
...
class LoopQueue(QueueBase):
    ...
    def enqueue(self, e):
        # 取余是因为比如tail直接在最后一个位置，front在idx=0;这样取余就正好相等
    ...
    def _resize(self, new_capacity):
        ...
        for i in range(self._size):
            # 仔细体会，循环遍历
            # capa:8, len(data):9, size:8, new_data:17, front:2, tail:1
            # 这里new_data从[0, size]赋值，对应的self._data从front开始获得，并且需要%len(self._data)取余
    ...
    def __str__(self):
        if self._tail >= self._front:
            return str('queue.LoopQueue : front {} tail, capacity: {}'.format(self._data[self._front:self._tail], self.get_capacity()))
        else:
            # presentation purpose only
            return str('queue.LoopQueue : front {} tail, capacity: {}'.format(str(self._data[self._front:] + self._data[:self._tail]), self.get_capacity()))
